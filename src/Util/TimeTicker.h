...
namespace toolkit {
    ...
    public:
         ...
         */
        Ticker(uint64_t min_ms = 0,LogContextCapture ctx = LogContextCapture(Logger::Instance(), LWarn, __FILE__, "", __LINE__),bool print_log = false) : _ctx(std::move(ctx)) {    //构造函数
            if (!print_log) {
                _ctx.clear();
            }
    ...
    public:
         ...
         */
        SmoothTicker(uint64_t reset_ms = 10000) {     //构造函数
            _reset_ms = reset_ms;
            _ticker.resetTime();
        }
