...
namespace toolkit {
    ...
    static UdpServer::PeerIdType makeSockId(sockaddr *addr, int) {
        return ((uint64_t) ((struct sockaddr_in *) addr)->sin_addr.s_addr) << 16 |
    ...
    void UdpServer::onRead_l(bool is_server_fd, const UdpServer::PeerIdType &id, const Buffer::Ptr &buf, sockaddr *addr,
                             int addr_len) {
    ...
    const Session::Ptr &
    UdpServer::getOrCreateSession(const UdpServer::PeerIdType &id, const Buffer::Ptr &buf, sockaddr *addr, int addr_len,
                                  bool &is_new) {
        {
            //减小临界区
            std::lock_guard<std::recursive_mutex> lock(*_session_mutex);
    ...
    const Session::Ptr &
    UdpServer::createSession(const PeerIdType &id, const Buffer::Ptr &buf, struct sockaddr *addr, int addr_len) {
        ...
        auto session_creator = [this, weak_self, socket, addr_str, id]() -> const Session::Ptr & {
            ...
            std::weak_ptr<Session> weak_session = session;
            socket->setOnRead(
                    [weak_self, weak_session, id](const Buffer::Ptr &buf, struct sockaddr *addr, int addr_len) {
                        auto strong_self = weak_self.lock();
                        if (!strong_self) {
                            return;
                        }
    ...
    void UdpServer::setOnCreateSocket(onCreateSocket cb) {
        if (cb) {
            ...
            _on_create_socket = [](const EventPoller::Ptr &poller, const Buffer::Ptr &buf, struct sockaddr *addr,
                                   int addr_len) {
    ...
    Socket::Ptr UdpServer::createSocket(const EventPoller::Ptr &poller, const Buffer::Ptr &buf, struct sockaddr *addr,
                                        int addr_len) {
