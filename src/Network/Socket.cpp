...
#define CLOSE_SOCK(fd) if(fd != -1) {close(fd);}
    ...
    void Socket::connect(const string &url, uint16_t port, onErrCB con_cb_in, float timeout_sec, const string &local_ip,
                         uint16_t local_port) {
        //重置当前socket
        closeSock();
        ...
        auto async_con_cb = std::make_shared<function<void(int)> >([weak_self, con_cb](int sock) {
            ...
            int result = strong_self->_poller->addEvent(sock, EventPoller::Event_Write,
                                                        [weak_self, weak_sock_fd, con_cb](int event) {
    ...
    bool Socket::attachEvent(const SockFD::Ptr &sock, bool is_udp) {
        ...
        int result = _poller->addEvent(sock->rawFd(),
                                       EventPoller::Event_Read | EventPoller::Event_Error | EventPoller::Event_Write,
                ...
                shared_ptr<void> completed(nullptr, [peer_sock, peer_sock_fd](void *) {
                    try {
                        ...
                        if (!peer_sock->attachEvent(peer_sock_fd, false)) {
                            ...
                            peer_sock->emitErr(
                                    SockException(Err_eof, "add event to poller failed when accept a socket"));
                        }
                    } catch (std::exception &ex) {
                        ErrorL << ex.what();
                    }
            ...
            do {
                {
                    ...
                    if (!_send_buf_waiting.empty()) {
                        ...
                        send_buf_sending_tmp.emplace_back(
                                std::make_shared<BufferList>(_send_buf_waiting, _send_result));
                        break;
                    }
                }
