...
///////////////BufferList/////////////////////
    ...
    int sendmsg(int fd, const struct msghdr *msg, int flags) {
        ...
        for (auto i = 0; i != msg->msg_iovlen; ++i) {
            do {
                n = sendto(fd, (char *) msg->msg_iov[i].iov_base, msg->msg_iov[i].iov_len, flags,
    ...
    BufferList::BufferList(List<std::pair<Buffer::Ptr, bool> > &list, SendResult cb) : _iovec(list.size()),
                                                                                       _cb(std::move(cb)) {
        _pkt_list.swap(list);
